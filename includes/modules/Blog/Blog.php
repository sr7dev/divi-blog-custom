...
class EHQ_Builder_Module_Blog extends ET_Builder_Module_Type_PostBased {
	/**
	...
	function init() {
		$this->name       = esc_html__( 'EHQ Blog', 'et_builder' );
		$this->plural     = esc_html__( 'EHQBlogs', 'et_builder' );
		$this->slug       = 'ehq_pb_blog';
	...
	function get_fields() {
		$fields = array(
			...
			/* Select second post type 
			 * 
			 * Bojana<bajana@sobetechguru.com>
			 * since 11/17/2019
			 */
			'append_second_post_type' => array(
				'label'             => esc_html__( 'Append Second Post Type', 'et_builder' ),
				'type'              => 'yes_no_button',
				'option_category'   => 'configuration',
				'options'           => array(
					'on'  => esc_html__( 'Yes', 'et_builder' ),
				...
				'description'       => esc_html__( 'Also display posts for the current page. Useful on archive and index pages.', 'et_builder' ),
				'computed_affects'   => array(
					'__posts',
				),
				'toggle_slug'       => 'main_content',
			...
			'post_type_2' => array(
				'label'             => esc_html__( 'Second Post Type', 'et_builder' ),
				'type'              => 'select',
				'option_category'   => 'configuration',
				'options'           => et_get_registered_post_type_options(),
				'description'       => esc_html__( 'Choose posts of which post type you would like to display.', 'et_builder' ),
				...
				'default'           => 'post',
				'show_if'           => array(
					'append_second_post_type' => 'on'
				),
			),
			/* Add Category for Second Post type
			 * 
			 * @author Bojana<bojana@sobetechguru.com>
			 * @since 11/17/2019
			 */
			'include_categories2' => array(
				'label'            => esc_html__( 'Included Categories of Second Post Type', 'et_builder' ),
				'type'             => 'categories',
				'meta_categories'  => array(
					'all'     => esc_html__( 'All Categories', 'et_builder' ),
					'current' => esc_html__( 'Current Category', 'et_builder' ),
			...
			'__posts' => array(
				'type' => 'computed',
				'computed_callback' => array( 'EHQ_Builder_Module_Blog', 'get_blog_posts' ),
				'computed_depends_on' => array(
					'use_current_loop',
					'post_type',
					/* Select second post type 
					 * 
					 * Bojana<bojana@sobetechguru.com>
					 * since 11/17/2019
					 */
					'append_second_post_type',
					'post_type_2',
					'fullwidth',
					'posts_number',
					'include_categories',
	 ...
	 *
	 * @param array   arguments that is being used by ehq_pb_blog
	...
	static function get_blog_posts( $args = array(), $conditional_tags = array(), $current_page = array() ) {
		...
		// Default params are combination of attributes that is used by ehq_pb_blog and
		...
		/*Bojana*/
		if ( $args['append_second_post_type'] == 'on' ) {
			$query_args2 = $query_args;
			$query_args2['post_type'] = $args['post_type_2'];
			$query_args2['cat'] = implode( ',', self::filter_include_categories( $args['include_categories2'], $post_id ) );
				...
				?>
					<article id="" <?php post_class( 'et_pb_post clearfix' . $no_thumb_class . $overlay_class ) ?>>
						...
						<?php if ( 'off' === $args['fullwidth'] || ! in_array( $post_format, array( 'link', 'audio', 'quote' ) ) ) { ?>
							...
							<?php
								if ( 'on' === $args['show_author'] || 'on' == $args['show_author_avatar'] || 'on' === $args['show_date'] || 'on' === $args['show_categories'] || 'on' === $args['show_comments'] ) {
									$author_avatar = '<div class="ds-avatar">'.get_avatar( get_the_author_meta( 'ID' ), 64 ).'</div>';
									$author = 'on' === $args['show_author']
										? et_get_safe_localization( sprintf( __( 'by %s', 'et_builder' ), ('on' == $args['show_author_avatar'] ? $author_avatar : '').'<span class="author vcard">' .  et_pb_get_the_author_posts_link() . '</span>' ) )
										: '';
	...
	function render_pagination( $echo = true ){
		...
		if ( ! $use_current_loop ) {
			add_filter( 'get_pagenum_link', array( 'EHQ_Builder_Module_Blog', 'filter_pagination_url' ) );
		}
		...
		if ( ! $use_current_loop ) {
			remove_filter( 'get_pagenum_link', array( 'EHQ_Builder_Module_Blog', 'filter_pagination_url' ) );
		}
	...
	function render( $attrs, $content = null, $render_slug ) {
		...
		// to its original state when ehq_pb_blog shortcode ends to avoid incorrect global $post
		 ...
		 * since 11/16/2019
		 */
		$append_second_post_type			 = isset( $this->props['append_second_post_type'] ) ? $this->props['append_second_post_type'] : 'off';
		$post_type_2                         = isset( $this->props['post_type_2'] ) ? $this->props['post_type_2'] : 'post';
		$fullwidth                           = $this->props['fullwidth'];
		$posts_number                        = $this->props['posts_number'];
		$include_categories2                  = $this->props['include_categories2'];
		...
		if ( 'off' === $use_current_loop ) {
			 ...
			 * @since 11/17/2019
			 */
			if ( $append_second_post_type == 'on' ) {
				global $wp_query;
		
				$args2 = $args;
				$args2['post_type'] = $post_type_2;
				$args2['cat'] = implode( ',', self::filter_include_categories( $include_categories2 ) );
	...
	public function multi_view_filter_value( $raw_value, $args, $multi_view ) {
		...
		if ( 'post_content' === $name && 'content' === $context ) {
			...
			if (isset( $post_meta_remove_keys['show_author_avatar']) ){
				$author_avatar = '<div class="ds-avatar">'.get_avatar( get_the_author_meta( 'ID' ), 64 ).'</div>';
				$post_meta_datas[] =$author_avatar;
			}

			if ( isset( $post_meta_remove_keys['show_author'] ) ) {
				
				$post_meta_datas[] = et_get_safe_localization( sprintf( __( 'by1 %s', 'et_builder' ), '<span class="author vcard">' . et_pb_get_the_author_posts_link() . '</span>' ) );
			}
